...
Outbound action flow is now lifecycle-bound too. Every qualified Bus/coordination command consumes an exact `AcceptedLifecyclePermit`; write-capable Bus commands are also single-use effect-fenced. PC jobs and generic provider effects dispatch only through `LifecycleEffectGateway`, where the effect fence binds the request to the accepted currentness/permit digest while keeping external authority evidence separate. Interrupted, blocked, memory-ahead, control-drift, and stale generations fail closed before execution. The raw coordination and PC/provider primitives remain low-level mechanisms, not the runtime-qualified escape path.\n\nIndependent falsification is deliberately different from internal assurance. Vera can contain DriftGuard-derived checking mechanisms while a separately executed DriftGuard remains useful specifically because it is outside Vera's own self-checking boundary.
